...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
  const summ = Math.sqrt((x2 - x1) ** 2 + (y2 - y1) ** 2);
  return summ;
}
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const deduction = x1 * x2 + y1 * y2;
  const formula = Math.sqrt(x1 ** 2 + y1 ** 2) * Math.sqrt(x2 ** 2 + y2 ** 2);

  const result = Math.acos(deduction / formula);
  return result;
}
 ...
 */
function isPrime(n) {
  for (let i = 2, numberCount = Math.sqrt(n); i <= numberCount; i += 1) {
